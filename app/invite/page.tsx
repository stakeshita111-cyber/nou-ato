...
export default function InvitePage() {
  ...
  const [name, setName] = useState("たろー");
  ...
  const handleLineAuth = () => {
    ...
    if (!email) {
      setEmail("example@nou-ato.jp");
    }
    setToastMessage("LINE連携が完了し、プロフィール情報を読み込みました");
    setShowToast(true);
  };
    ...
    <div className="min-h-screen bg-[#f7f9f5] flex flex-col items-center py-6 px-4 font-sans text-gray-800">
      ...
      <div className="w-full max-w-md space-y-5">
        ...
        <form onSubmit={handleJoin} className="space-y-4">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 space-y-4">
            <div>
              ...
              />
              <div className="flex items-start gap-1.5 mt-2 text-[#1c4d21] text-[11px] leading-tight">
                <svg className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>LINE名が自動入力されています。本名に修正してください。</span>
              </div>
            </div>
        ...
        <p className="text-[11px] text-center text-gray-500 leading-relaxed pt-1">
          登録することで、
          <a href="#" className="underline text-gray-600 hover:text-green-800">利用規約</a>
          および
          <a href="#" className="underline text-gray-600 hover:text-green-800">プライバシーポリシー</a>
          に同意したものとみなされます。
        </p>
      </div>
    </div>
