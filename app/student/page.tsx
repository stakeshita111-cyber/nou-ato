...
export default function StudentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // tasks(*) を結合して、講師が登録した最新の詳細情報をすべて引っ張る
  ...
  return (
    <div className="p-4 md:p-8 max-w-md mx-auto bg-gray-50 min-h-screen pb-20">
          ...
          {tasks.map((task) => {
            ...
            return (
              <div key={task.id} className="min-w-[85%] md:min-w-[300px] snap-center shrink-0 p-5 border rounded-2xl shadow-md bg-white relative">
                ...
                <p className="text-sm text-gray-500 mt-1 font-bold">★{displayDifficulty} / {displayExp} EXP</p>
      ...
      {selectedTask && (() => {
        // tasks テーブル側の最新データを優先的に参照
        const taskDetails = selectedTask.tasks || {};
        const title = taskDetails.title || selectedTask.title;
        const exp = taskDetails.exp ?? selectedTask.exp ?? 10;
        const difficulty = taskDetails.difficulty ?? selectedTask.difficulty ?? 1;
        const estimatedTime = taskDetails.estimated_time || selectedTask.estimated_time;
        const toolsNeeded = taskDetails.tools_needed || selectedTask.tools_needed;
        const description = taskDetails.description || selectedTask.description;
        const memo = taskDetails.memo || selectedTask.memo;
        const referenceLinks = taskDetails.reference_links || selectedTask.reference_links;
        const requirePhoto = taskDetails.require_photo ?? selectedTask.require_photo;
              ...
              <div className="p-6 space-y-6">
                <div>
                  ...
                  <div className="flex gap-2">
                    <span className="text-sm bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-bold">EXP: {exp}</span>
                    <span className="text-sm bg-orange-100 text-orange-800 px-3 py-1 rounded-full font-bold">難易度: {difficulty}</span>
                    {estimatedTime && (
                      <span className="text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded-full font-bold">目安: {estimatedTime}</span>
                    )}
                  </div>
                </div>
                ...
                {toolsNeeded && (
                  <div>
                    ...
                    <p className="bg-gray-50 p-3 rounded-lg border">{toolsNeeded}</p>
                  </div>
                )}
                ...
                {referenceLinks && (
                  <div>
                    ...
                    <a href={referenceLinks} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline break-all">
                      {referenceLinks}
                    </a>
                  </div>
                )}
